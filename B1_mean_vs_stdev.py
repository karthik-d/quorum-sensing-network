...
for feedback_str in ["", "noneg"]:
	...
	for levels_fpath in levels_l:
		...
		plot.savefig(os.path.join(
			...
			f'kdtree_{feedback_str}_local-mean-stdev_levels_select-{str(seeding_density).ljust(4, '0')}.png'), dpi=100)
		plot.close()
	...
	plot.savefig(os.path.join(
		f'analysis_outputs/kdtree/{feedback_str}', 
		f'kdtree_{feedback_str}_local-mean-stdev_levels_overlay.png'), dpi=100)
	plot.close()
	...
	plot.savefig(os.path.join(
		f'analysis_outputs/kdtree/{feedback_str}', 
		f'kdtree_{feedback_str}_local-mean-stdev_clouds_overlay.png'), dpi=100)
	plot.close()